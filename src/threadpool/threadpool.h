...
typedef struct {
	...
	pthread_mutex_t mutex;		//The mutex that protects all the data
	...
	pthread_cond_t new_work;	//Condition variable used to notify the workers if there is
								//new work available, or if they should stop
	pthread_cond_t task_done;	//Notify when a worker has finished a task
} threadpool_t;
